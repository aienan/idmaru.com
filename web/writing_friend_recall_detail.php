	...
	if($status==4){//전체보기일 경우
		if($keyword != "thereisnokeyword" && $category=="writer"){//키워드 검색이 작성자일경우
			...
			if($friend_check==1){//친구가 맞을경우
				if($news_type=="all"){$sql4 = "SELECT * FROM $table_name WHERE user_num=$sql_fetch_array9[user_num] AND status!='3'";}
			...
			if($news_type=="all"){$sql4 = "SELECT * FROM $table_name WHERE status!='3' AND $mysql_friend_check";}
