<?php
	...
	include 'writing_reply_recall_detail.php';
	if($count_link != 0){//세부목록 보기일때
		...
		if($count4 != 0){
			...
			if($keyword=="thereisnokeyword"){
				$output .= '<div class="empty_list">댓글을 쓴 뉴스가 없습니다</div>';
		...
		if($count_link != 0){//세부목록으로 들어갈 경우
			...
			$html .= '
				...
				<script>
					...
					$("#writing_area_article'.$sql_fetch_array1["count"].' .writing_C").height(height-4);
