...
?>
	<script>
		var type_name = "news";//php_tile_list에서 사용
		var display_number = 40;
		var url = "<?php echo $url;?>";
		var url_all = "news_all.php?";//검색시에 전체뉴스로 이동
<?php
	if(isset($_REQUEST["news_type"])){//news_type 변수가 넘어왔을때
		echo 'var news_type = "'.$_REQUEST["news_type"].'";';
	...
	if(isset($_REQUEST["order_column"])){//order_column 변수가 넘어왔을때
		echo 'var order_column = "'.$_REQUEST["order_column"].'";';
...
?>
		...
		$(document).ready(function () {
			...
			$.ajax({
				...
				data: {count_start:count_start, display_number:display_number, height_total:height_total, width_total:width_total, news_type:news_type, order_column:order_column},
				success: function(getdata){
					$("#news_article_area").append(getdata);
					$("#loading_more").remove();
				}
			});
		});
	...
	<div id="bodyarea">
		<div id="mainbar">
			<div id="mainbody">
				...
				<div id="news_article_area" class="tile_area"></div>
			</div>
		</div>
