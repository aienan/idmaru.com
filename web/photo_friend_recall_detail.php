	...
	if($status==4){//전체보기일 경우
		if($keyword != "thereisnokeyword" && $category=="writer"){//키워드 검색이 작성자일경우
			...
			if($friend_check==1){//친구가 맞을경우
				if($gallery_type=="all"){$sql4 = "SELECT * FROM $table_name WHERE user_num=$sql_fetch_array9[user_num] AND status!='3'";}
				else {$sql4 = "SELECT * FROM $table_name WHERE user_num=$sql_fetch_array9[user_num] AND status!='3' AND type=$type";}
			...
			if($gallery_type=="all"){$sql4 = "SELECT * FROM $table_name WHERE status!='3' AND $mysql_friend_check";}
			else {$sql4 = "SELECT * FROM $table_name WHERE status!='3' AND $mysql_friend_check AND type=$type";}
		...
		if($keyword != "thereisnokeyword" && $category=="writer"){//키워드 검색이 작성자일경우
			...
			$mysql_friend_check = mysql_friend_check("user_num");
			$sql4 = "SELECT * FROM $table_name WHERE status='$status' AND $mysql_friend_check";
		} else if($keyword != "thereisnokeyword" && $category != "writer"){//작성자 검색이 아니고 keyword 검색을 했을 때
			$friend_array = explode(";",$_SESSION["friend_list"]);
			$sphinx->SetFilter("user_num", $friend_array );
