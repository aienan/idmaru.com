<?php
	...
	if($status==4){//전체보기일 경우
		if($keyword == "thereisnokeyword" && $category == "thereisnocategory"){//keyword 검색을 하지 않았을 때
			if($news_type=="all"){$sql4 = "SELECT * FROM $table_name WHERE user_num=$user_num";}//list_show를 만들기 위해
		...
		if($keyword == "thereisnokeyword" && $category == "thereisnocategory"){//keyword 검색을 하지 않았을 때
			if($news_type=="all"){$sql4 = "SELECT * FROM $table_name WHERE user_num=$user_num AND status='$status'";}//list_show를 만들기 위해
