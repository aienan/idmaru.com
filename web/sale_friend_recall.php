<?php
	...
	include 'sale_friend_recall_process.php';
		...
		if($count4 != 0 || $status != 4){//글이 있거나 전체보기가 아닐경우
			...
			$output .= '
				...
				<script>
					...
					$("#sale_friend_list_D").hover(function(){//공개여부 영역의 글씨 색깔을 바꿈
	...
	if($count4 != 0){//결과물이 있을경우
		for($i=0; $i<$count1; $i++){//개별 결과 출력
			...
			} else if(($keyword != "thereisnokeyword" && $category != "writer") || ($selling_type != "all")){//작성자 검색이 아니고 keyword 검색을 했거나 selling_type을 설정했을때
			...
			if($count_link != 0){//세부목록으로 들어갈 경우
				...
				$html .= '
					...
					<div id="info_area">
						...
						<div id="info_area_status"> 거래상태 : <span id="write_content_status" class="inline">'.$status.'</span>
						</div>
