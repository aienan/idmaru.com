...
if($count_link != 0){//세부목록일때
	...
	if($count4 != 0){//결과물이 있을때
		$output .= '
			...
			<script>
				$("#list_number, #list_search").css("display", "block");
				$("#photo_watch").css("display", "block");
			</script>
		';
...
for($i=0; $i<$count1; $i++){//photo 출력
	...
	else if($count_link==0) {// 사진이 있고 목록을 출력할 때
	...
	if($count_link != 0){//사진을 한장만 출력할 경우
		...
		$html .= '
			<div id="photo_'.$sql_fetch_array1["count"].'"><a href="'.$url.'count_link='.$sql_fetch_array1["count"].'&list_count='.$list_count_link.$number_url.$keyword_url.$category_url.'&gallery_type='.$gallery_type.'"><img src="'.$photo_path.'" width="'.$image_width_adjusted.'" height="'.$image_height_adjusted.'" style="margin:0px 10px 0px 0;" /></a></div>
			<script>
