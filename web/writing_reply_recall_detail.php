<?php
	...
	for($i=0; $i < count($reply_arr_all); $i++){
		if($news_type=="all"){$sql12 = "SELECT * FROM $table_name WHERE count=$reply_arr_all[$i] AND user_num != $user_num AND status != '3'";}
	...
	if($keyword != "thereisnokeyword" && $category=="writer"){//키워드 검색이 작성자일경우
		...
		for($i=0; $i < count($reply_arr); $i++){
			if($news_type=="all"){$sql15 = "SELECT * FROM $table_name WHERE count=$reply_arr[$i] AND user_num=$sql_fetch_array9[user_num] AND status != '3'";}
