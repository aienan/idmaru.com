<?php
	...
	include 'event_all_recall_process.php';
		...
		if($count4 != 0 || $status != 4){//글이 있거나 전체보기가 아닐경우
			$output .= '
				<div id="list_head">
					...
					<div id="club_list_title">이&nbsp;&nbsp;&nbsp;&nbsp;벤&nbsp;&nbsp;&nbsp;&nbsp;트</div>
					<div id="club_list_count">조회수</div>
					<div id="club_list_status">
						<span class="pointer">▼ 종류</span>
				...
				<script>
					$("#apply_button").click(function(){
						...
						if(status != 4){location.href = "'.$url.'list_count=1'.$number_url.$keyword_url.$category_url.'&status="+status;}
						else {location.href = "'.$url.'list_count=1'.$number_url.$keyword_url.$category_url.'";}
					});
			...
			if($status==4){//전체 보기일 경우
				$output .= '<script>$("input:radio[name=status]:nth(0)").attr("checked", true);</script>';
			} else if($status==1){//행사 보기일 경우
				$output .= '<script>$("input:radio[name=status]:nth(1)").attr("checked", true);</script>';
			} else if($status==2){//개인 보기일 경우
				$output .= '<script>$("input:radio[name=status]:nth(2)").attr("checked", true);</script>';
			} else if($status==3){//봉사 보기일 경우
				$output .= '<script>$("input:radio[name=status]:nth(3)").attr("checked", true);</script>';
			}
		}
		if($count4 != 0){
			...
			if($keyword!="thereisnokeyword"){//키워드가 있을 경우
				$output .= '<script>$("#list_search").css("display", "block");</script>';
			}
		}
	}
	for($i=0; $i<$count1; $i++){//검색결과 출력
		...
		$sql6 = "SELECT * FROM user WHERE user_num = $sql_fetch_array1[user_num]";//작성자 정보를 알아내기 위함
		...
		if($count_link != 0){//세부목록으로 들어갈 경우
			...
			<div id="main_writing">
				...
				';
			if($user_num==$sql_fetch_array1["user_num"]){//글쓴이 본인일경우
				$html .='
			...
			$html .= '
				<div id="club_area_article'.$sql_fetch_array1["count"].'" class="writing_area_article">
			...
			$html .= '
					...
					<a href="'.$url.'count_link='.$sql_fetch_array1["count"].'&list_count='.$list_count_link.$number_url.$keyword_url.$category_url.$status_url.'"><div class="club_title_event">'.$title.' &nbsp;<span style="color:#3C847F; font-weight:normal;">('.$count5.')</span>&nbsp;'.$updown_total_string.'</div></a>
					<div class="club_list_count"><div style="padding:9px;">'.$sql_fetch_array1["read_count"].'</div></div>
					<div class="club_status_event"><div style="padding:7px;">'.$status.'</div></div>
				</div>
				<script>
					...
					$("#club_area_article'.$sql_fetch_array1["count"].' .club_list_count").height(height);
