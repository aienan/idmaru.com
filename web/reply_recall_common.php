<?php
	...
	if($table_name=="sale"){
		for($i=0; $i < count($reply_arr_all); $i++){
			if($selling_type=="all"){$sql12 = "SELECT * FROM $table_name WHERE count=$reply_arr_all[$i] AND user_num != $user_num";}
			else {$sql12 = "SELECT * FROM $table_name WHERE count=$reply_arr_all[$i] AND user_num != $user_num AND type=$type";}
			$sql_query12 = mysqli_query($conn, $sql12);
			$count12 = mysqli_num_rows($sql_query12);
			if($count12){
				$sql_fetch_array12 = mysqli_fetch_array($sql_query12);
				if($sql_fetch_array12["status"]==1 || $sql_fetch_array12["status"]==3){$reply_arr[] = $reply_arr_all[$i];}
			}
		}
	} else if($table_name=="club_event"){
		for($i=0; $i < count($reply_arr_all); $i++){
			if($status_num==4){$sql12 = "SELECT * FROM $table_name WHERE count=$reply_arr_all[$i] AND user_num != $user_num";}
			else {$sql12 = "SELECT * FROM $table_name WHERE count=$reply_arr_all[$i] AND user_num != $user_num AND status='$status_num'";}
			$sql_query12 = mysqli_query($conn, $sql12);
			$count12 = mysqli_num_rows($sql_query12);
			if($count12){$reply_arr[] = $reply_arr_all[$i];}
		}
	}
	if($keyword != "thereisnokeyword" && $category=="writer"){//키워드 검색이 작성자일경우
		...
		for($i=0; $i < count($reply_arr); $i++){
			$sql15 = "SELECT * FROM $table_name WHERE count=$reply_arr[$i] AND user_num=$sql_fetch_array9[user_num]";
