<?php
	...
	$status_num = $status;
	if($table_name=="sale" && ($selling_type != "all" || !$selling_type)){//거래에서 selling_type이 설정되었을 때
		if($keyword=="thereisnokeyword"){$keyword = $selling_type; $category="title"; $search_field = "title";}//키워드 검색을 안했을때
	}
	if($status==4){//전체보기일 경우
		if($keyword == "thereisnokeyword" && $category == "thereisnocategory"){//keyword 검색을 하지 않았을 때
			$sql4 = "SELECT * FROM $table_name WHERE user_num=$user_num";//list_show를 만들기 위해
		...
		if($keyword == "thereisnokeyword" && $category == "thereisnocategory"){//keyword 검색을 하지 않았을 때
			$sql4 = "SELECT * FROM $table_name WHERE user_num=$user_num AND status='$status'";//list_show를 만들기 위해
		} else if($keyword != "thereisnokeyword"){//keyword 검색을 했을 때
			$sphinx->SetFilter("user_num", array($user_num) );
