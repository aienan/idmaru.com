		...
		if($count4 != 0 || $status != 4){//글이 있거나 전체보기가 아닐경우
			$output .= '
				...
				<script>
					...
					$("#club_list_status").hover(function(){
	...
	for($i=0; $i<$count1; $i++){//검색결과 출력
		...
		if($count_link != 0){//세부목록으로 들어갈 경우
			...
			$html .= '
					...
					<div class="club_list_count">'.$sql_fetch_array1["read_count"].'</div>
