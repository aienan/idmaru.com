<?php
	...
	if($count_link != 0){//세부목록 보기일때
		...
		if($count4 != 0){
			$output .= '
				<div id="list_head">
					<div id="market_list_seller">판&nbsp;매&nbsp;자</div>
					<div id="market_list_title">물&nbsp;&nbsp;&nbsp;품&nbsp;&nbsp;&nbsp;명</div>
					<div id="market_list_read">조회수</div>
				</div>
			';
		} else if($count4==0){
			if($keyword=="thereisnokeyword"){
				$output .= '
					<div class="empty_list">등록된 물품이 없습니다</div>
				';
	...
	if($count4 != 0){//결과물이 있을경우
		for($i=0; $i<$count1; $i++){//개별 결과 출력
			$html = '';
			if(($keyword != "thereisnokeyword" && $category=="writer") || ($keyword == "thereisnokeyword" && $category == "thereisnocategory")){
				$sql_fetch_array1 = mysqli_fetch_array($sql_query1);
			} else if($keyword != "thereisnokeyword" && $category != "writer"){//작성자 검색이 아니고 keyword 검색을 했을때
			...
			if($count_link != 0){//세부목록으로 들어갈 경우
				...
				$html .= '
					<div id="writing_area_article'.$sql_fetch_array1["count"].'" class="writing_area_article">
						<div class="market_list_seller">'.$sql_fetch_array6["user_name"].'</div>
						<a href="'.$url.'count_link='.$sql_fetch_array1["count"].'&list_count='.$list_count_link.$number_url.$keyword_url.$category_url.'&selling_type='.$selling_type.'"><div class="market_list_title">'.$title.' &nbsp;<span style="color:#3C847F; font-weight:normal;">('.$count5.')</span></div></a>
						<div class="market_list_read">'.$sql_fetch_array1["read_count"].'</div>
					</div>
					<script>
						var height = $("#writing_area_article'.$sql_fetch_array1["count"].' .market_list_title").height();
						$("#writing_area_article'.$sql_fetch_array1["count"].'").height(height+10);
						$("#writing_area_article'.$sql_fetch_array1["count"].' .market_list_seller").height(height);
						$("#writing_area_article'.$sql_fetch_array1["count"].' .market_list_title").height(height);
						$("#writing_area_article'.$sql_fetch_array1["count"].' .market_list_read").height(height);
					</script>
				';
			}
			$output .= $html;
